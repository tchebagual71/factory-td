...
32. `[~]` **Logistics recovery & routing tools** — the biggest playability gap. Partly shipped:
    ...
    - **Still open:** filter and priority splitters, overflow routing, and a buffer/storage building. Those are the *systemic* fix — they stop the wrong item entering the wrong line at all, rather than helping you clean up after it — and each is a new building, so palette slot, hotkey (mind `keymap.test.ts`), cost, art, save round-trip and tests
...
34. `[~]` **Research variety + the coolant arbitrage** — (b) **shipped**, (a) still open.
    ...
    - **Still open (a):** most cards are flat percentage multipliers, so the draw is rarely a decision. Wants tower/machine-*changing* cards, mutually exclusive branches, rerolls or locks, and cards touching splitter behaviour, buffers, recipes or map economy
35. `[ ]` **Wave & boss variety** — every wave is a single enemy type and bosses are mostly larger stat blocks, so once the counter table is learned the decision collapses to "build the tower the preview names". Wants mixed compositions within a wave, bosses with mechanics (shielding neighbours, disabling belts, splitting, changing route behaviour), map modifiers and difficulty modes, challenge goals (limited build area, limited ore, no selling, timed waves), and an explicit endless milestone or victory objective instead of only an eventual throughput wall. **Constraint: `waves.test.ts` pins the early ramp, the post-taper growth ceiling, the wave-20 floor and income-vs-threat pacing — those invariants are load-bearing and must survive**
36. `[ ]` **Adaptive 3D quality** — `IsoView` combines antialiasing, up to 2× DPR, real-time shadows, a 2048² shadow map, a full display-list scan per frame and separate body/lid meshes. That is a lot to ask of a high-DPI phone with a late-game factory on screen. Wants a mobile preset (DPR 1, no shadows, smaller map), frame-time-driven adaptive quality, WebGL context-loss handling, optionally a 30fps cap on the 3D mirror while the sim stays 60, and automatic fallback to flat after sustained bad frames. The fallback path already exists for WebGL init failure
...
38. `[ ]` **Cloud write ordering + leaderboard integrity** — `pushSave` is a fire-and-forget unconditional upsert, so two overlapping requests can land out of order and an older factory can overwrite a newer one; wants a `savedAt`-conditional server-side update or a single client-side queue. Separately the browser can upsert any `best_wave`: ownership RLS stops you editing *someone else's* row but cannot establish that a score came from real play. **The schema and policies are not in `src` and cannot be verified from the client — audit them in the database directly**, including `security_invoker` on the exposed `leaderboard` view, and have the query state its own ordering rather than trusting the view's
39. `[ ]` **Cross-device meta progression** — Workshop scrap and per-node levels are local-only by design (`state/meta.ts`), and lifetime stat *counters* are local too; only completed achievement ids and best wave merge today. A signed-in player on a second device silently starts the Workshop over. Wants scrap + node levels synced with a max-per-node merge rule in `mergeProgress.ts` (the `jsonb` column on `profiles` noted in 22), and stat counters with explicit per-stat merge rules — max for high-water marks, and a decision on whether cumulative counters sum (double-counts a shared run) or take the max (loses concurrent progress)
